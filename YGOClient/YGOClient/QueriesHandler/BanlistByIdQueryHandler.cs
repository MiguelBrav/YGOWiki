...
{
    public class BanlistByIdQueryHandler : IRequestHandler<BanlistByIdQuery, ApiResponse>
    {
        ...
        public BanlistByIdQueryHandler(YGOWiki.YGOWikiClient client, ICacheService cacheService, IConfiguration config, IMemoryCache cache)
        {
            _client = client;
        ...
        public async Task<ApiResponse> Handle(BanlistByIdQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                BanlistTypeDetail response = await _client.GetTypeBanlistAsync(new ByLanguageIdAndId { LanguageId = request.LanguageId, Id = request.Id });
