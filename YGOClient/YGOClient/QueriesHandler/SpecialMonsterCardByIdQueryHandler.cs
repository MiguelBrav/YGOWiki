...
namespace YGOClient.QueriesHandler
{
    public class SpecialMonsterCardByIdQueryHandler : UseCaseBase<SpecialMonsterCardByIdQuery, ApiResponse>
    {
        ...
        public SpecialMonsterCardByIdQueryHandler(YGOWiki.YGOWikiClient client, ICacheService cacheService, IConfiguration config, IMemoryCache cache)
        {
            _client = client;
        ...
        public override async Task<ApiResponse> Execute(SpecialMonsterCardByIdQuery request)
        {
            ...
            try
            {
                ...
                SpecialMonsterTypeDetail response = await _client.GetSpecialMonsterTypeAsync(new ByLanguageIdAndId { LanguageId = request.LanguageId, Id = request.Id });
