...
{
    public class RarityByIdQueryHandler : IRequestHandler<RarityByIdQuery, ApiResponse>
    {
        ...
        public RarityByIdQueryHandler(YGOWiki.YGOWikiClient client, ICacheService cacheService, IConfiguration config, IMemoryCache cache)
        {
            _client = client;
        ...
        public async Task<ApiResponse> Handle(RarityByIdQuery request, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                RarityTypeDetail response = await _client.GetRarityTypeAsync(new ByLanguageIdAndId { LanguageId = request.LanguageId, Id = request.Id });
